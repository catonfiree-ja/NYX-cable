...
export async function generateMetadata(): Promise<Metadata> {
  ...
  return {
    title: privacy?.metaTitle || 'นโยบายความเป็นส่วนตัว | Privacy Policy | NYX Cable',
    description: privacy?.metaDescription || 'นโยบายความเป็นส่วนตัวของ NYX Cable — การเก็บรวบรวม ใช้ และปกป้องข้อมูลส่วนบุคคลของผู้ใช้งาน',
  }
}
...
export default async function PrivacyPolicyPage() {
  ...
  const lastUpdated = privacyCms?.lastUpdated || 'มีนาคม 2026'
  const hasCmsBody = privacyCms?.body && privacyCms.body.length > 0
  ...
  return (
    <>
      ...
      <div className="pp-content">
        {hasCmsBody ? (
          <PortableText value={privacyCms.body} />
        ) : (
          <>
        ...
        <p style={{ fontSize: '0.82rem', color: '#94a3b8', marginTop: 32 }}>ปรับปรุงล่าสุด: {lastUpdated}</p>
      </div>
    </>
  )
}
